...
| **Workers** (N) | mid (varies by task) | Execute one task per container (operational through gate/verification/landing): checkout task branch, run workloads, write artifacts (task branch + HF mount), push, report status, write reports + retrospectives | Approve their own gates; push outside their task branch; perform unsolicited adversarial review |
...
*Verification, not adversarial review:* gate verification checks **formal submission criteria** (criteria met, claims evidenced and reasonable) — it is not an adversarial research review. **Adversarial review is deliberately not a framework role**: when a worker or the director wants one, they commission it as an ordinary task (a graph activity / peer-review task) that flows through the same gates.
