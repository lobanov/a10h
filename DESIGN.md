...
- **HuggingFace Hub:** checkpoints, datasets, and other large artifacts; pointers recorded in the repo tree. Artifact lineage (path ↔ git commit ↔ HF revision) is indexed by the secretary.
...
- SSE instruction event schema: envelope (kind, job/branch correlation, payloads incl. rebase target SHA and secretary operational details), ack semantics, offer/accept handshake for work assignment (R4).
