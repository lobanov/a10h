...
A multi-agent system (MAS) that operates a research lab for **experimental research on AI model architectures**. The MAS is **infrastructure, not the subject**: it plans, runs, verifies, and reflects on real experiments (training small models, evals, sweeps, analysis) across local GPUs and remote APIs, while a single human operator supervises through a web dashboard.
...
| 3 | Reflective learning & audit workflows | Exit-gate verification (secretary), worker retrospectives, reflector agent, director approval ladder |
...
| 6 | Agent hierarchy (director, workers, secretary, reflector) | pi SDK sessions in containers; roster in §5 |
...
```
  ...
  operator ──browser──► │  Dashboard (web UI: ops view + approval inbox [chat: M8])                          │
                        ...
                        │     ▼                        │   director (live) · reflector (M9)               │
                        │  Postgres (lightweight       │   secretary (R6: handoff + gate verification)    │
                        │  state + lineage index)      │                                                 │
                        │                              │                                                 │
...
**Why:** with the hub as origin, the hub always holds the latest committed state the moment a worker pushes; the secretary's verification and mechanical gates can therefore operate on *committed* state hub-side, and dependent tasks build on merged main — no evidence blob uploads, no injection plumbing.

**Branch governance (task branches + verified-complete merge):**
...
- A **pre-receive hook** (policy file generated by the hub) accepts a push only if: the ref matches the job's task branch, the pusher token matches, and the push is a **fast-forward** — pushes to any other ref, including `main`, are **denied server-side**. One carve-out: a **hub-granted, one-time rebase/force authorization** on that task ref (see landing below). Repair iterations and lease-expiry requeues **append** on the same branch — partial dead work stays visible for audit. The hook is thin: it calls a supervisor API that validates the push and **atomically consumes** any one-time authorization (no shared-policy-file races); a consumed-but-failed push re-grants automatically at the next landing attempt. Ref **deletions and tag pushes are denied outright**. **Operator write path — GitHub remote, no bespoke tokens:** each project repo's **canonical upstream is its GitHub remote**; the operator commits and pushes there with normal GitHub credentials (`goal.md`, graph revisions, human fixes, PR review). The hub's bare repo is origin-for-workers and **syncs with the GitHub remote**: fetch + fast-forward `main` through the serialized per-repo writer, and publish landed merges back (deploy-key/PAT auth — existing GitHub mechanisms). Non-ff divergence surfaces as an operator-visible event rather than auto-resolving. Hub-side writers (merges, secretary notes, director plan revisions) commit through the same serialized per-repo writer.
- **Verified-complete merge:** `main` advances only after the task passes its exit gate **and** the secretary's formal verification passes. Landing is **serialized per repo**. If the branch tip is a descendant of current `main` → **fast-forward merge**. Otherwise (concurrent branches moved `main`) → the hub **SSE-instructs the owning worker to fetch, rebase onto updated `main`, and push** (worker-side work; the rebase push is non-ff by definition, hence the one-time force authorization), followed by **another verification round** on the new tip, then merge. The instruction always has a recipient: the worker remains operational until the merge (§4).
...
- **Removed by this refinement:** the evidence-upload API and the `inputs_evidence` injection — gates and the secretary read evidence from the bare repo (committed state), and dependent tasks receive upstream evidence by cloning `main` after merges. Workers access repos exclusively through the gitserver (clone/fetch/push); nothing durable ever lives only inside a worker container.
...
- **Project repo (per research project, private/public case-by-case):** goal, planning graphs, experiment code, task-specific worker skills, artifacts. Canonical upstream on **GitHub** (the operator's write path — normal credentials); mirrored hub-side by the gitserver (§3.2.1), which is origin for workers: they do a **full in-container checkout per task** (clone from the gitserver URL with CA + token; checkout the task branch; delete on task end). **No worktrees; no `/repo` bind mount on workers.**
...
- **Skill placement:** role-shaping skills (secretary, reflector personas — hub-side role concerns) live in the **framework repo**; project repos carry only **worker-task-specific skills** (the demo carries a single one: `skills/demo/`). Role-shaping skills are authored framework-side — secretary in R6, reflector in R7.
...
- **Role→model mapping is configuration** (`DIRECTOR_MODEL`, `AUDITOR_MODEL` — the latter becomes `SECRETARY_MODEL` when the auditor folds in at R6; per-project mapping in `config/project.yaml`), backed by a **model-tier registry** that also drives the escalation ladder's "second opinion from a more capable model."
...
- **Worker lifecycle (R-series):** one task per container — a worker starts, registers (well-known URL → session id + per-session SSE endpoint), waits for instructions, and executes exactly one task **through its full lifecycle**: checkout → work → push → gate/verification → repair/rebase as instructed → merge. It stays operational until the **secretary signals exit-eligibility** (after the verified-complete merge, or after a failed attempt is closed and its summary note committed), then exits; compose `restart: always` recreates it. Damage does not accumulate across tasks by construction. A worker that dies mid-task is never rescued — its lease expires and the task requeues from scratch. Idle = subscribed and waiting (no polling loop).
...
| **Workers** (N) | mid (varies by task) | Execute one task per container (operational through gate/verification/landing): checkout task branch, run workloads, write artifacts (task branch + HF mount), push, report status, write reports + retrospectives | Approve their own gates; push outside their task branch |
| **Secretary** | mid | **Work handoff + formal gate verification + records:** operationalizes director intent into worker-facing specifics (artifact paths/refs, follow-up requests — retrospective prompts, summarize-work requests, archiving instructions) and **executes** them; at each exit gate verifies **formal submission criteria** — criteria met, claims evidenced and reasonable — hub-side on committed state (the reported SHA in the bare repo, incl. post-rebase re-verification); retention executor (preserve branch + note to main; **owns the worker exit signal** — post-merge / attempt-closure); taxonomies/indices/lineage | Decide research direction; perform adversarial research review |
| **Reflector** | strong | Cross-campaign reflection: proposes changes to plans and **skills** from accumulated retrospectives/verifications | Apply changes directly |
...
3. During execution, the director may revise plans autonomously **iff** the goal is unchanged **and** gate verification is passing.
...
2. **Mechanical gates** read exactly that SHA from the bare repo (never "the tip" — appends/repairs can't shift what gets verified) and resolve criteria against the job's declared `outputs.evidence` paths — **no tree-wide search**, so a sabotaged worker can't satisfy a criterion with a decoy file. The **secretary** verifies the **formal submission criteria** on the same committed state: criteria met, claims evidenced and reasonable.
3. Pass → task is **verified-complete**; the hub merges the task branch into `main` (§3.2.1). Fail → routes back to the worker for **repair** (SSE instruction; next iterations append on the same branch); after **two failed repairs on the same deficiency the hub itself escalates** to the director (hub-enforced loop bound, not worker discipline).
...
2. Reflector aggregates retrospectives + verification anomalies and proposes plan/skill changes.
...
- **Evidence-first:** evidence is **committed to the task branch**; gates and the secretary read it from the bare repo (hub-side git). Upstream evidence reaches dependent tasks through **merged `main`** — no upload API, no injection.
...
- **Gate-verification feedback** on gate failures — the worker's Pi consumes it as a **turn input** for the repair iteration.
...
| Risk | Mitigation |
|---|---|
| Verification becomes a rubber stamp | Evidence is committed state verified pre-merge (verified-complete gate to main); verification checks are skill-driven and versioned; main never holds unverified work |
...
- **Git-plane event taxonomy:** branch-created / pushed / verified-complete / merged / note-committed event schema for the dashboard feed (emitted R7, rendered M7).
...
- GitHub sync mechanics: sync trigger (webhook vs poll vs on-demand pre-promotion fetch), publish-back cadence for landed merges, divergence handling beyond the operator-visible event.
- Rebase churn policy parameters: concurrency threshold and hold-off timing for deferred landings (R2/R5 defaults).
- Hub-side git access for gates/secretary: git CLI in the hub container vs libgit2-style library (R5 implementation choice).
- Secretary agent scope (R6): prompt/tool surface; verification surface (formal submission checks); which follow-up request types exist at first release.
