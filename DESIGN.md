...
```
  ...
  operator ──browser──► │  Dashboard (web UI: ops view + approval inbox [chat: M8])                          │
                        ...
                        │  Postgres (lightweight       │   librarian (secretary, R6) · reflector (M9)     │
                        │  state + lineage index)      │                                                 │
                        │                              │                                                 │
        ...
        branch (git over TLS)  │         │ branch only   │ session id → per-session     │ progress, acks
                    ┌──────────▼─────────┴──┐       ┌───▼───────────────────┐         │
                    │ WORKER (one task,     │◄──────┘ per-container SSE    │◄────────┘
                    │ then exit; restart:   │         endpoint, idle-waits │
                    │ always)               │                                   │
...
- **Retention — always preserve:** failed-task branches are never deleted. For **every** attempt the librarian commits a **summary note to `main`** (what was attempted, gate/audit outcome, links to the branch), so the repo itself carries the attempt history. HF artifact deletion remains a separate, explicit decision (default: retain).
...
- **HuggingFace Hub:** checkpoints, datasets, and other large artifacts; pointers recorded in the repo tree. Artifact lineage (path ↔ git commit ↔ HF revision) is indexed by the librarian.
- **Postgres (hub only, lightweight):** operational state and lookups — jobs, leases, progress events, plans/activities, approvals, gate verdicts, agent log, nodes — plus the **lineage index** (formerly the artifacts content store). The dashboard is a live projection of this state; **no research content lives in Postgres**.
...
- **Hub compose stack:** `supervisor`, `postgres`, `dashboard`, `gitserver` (+ optional `litellm`). GPU services optional.
...
- **Worker lifecycle (R-series):** one task per container — a worker starts, registers (well-known URL → session id + per-session SSE endpoint), waits for instructions, executes exactly one task (checkout → work → push → status), then **exits**; compose `restart: always` recreates it. Damage (state, memory, filesystem) does not accumulate across tasks by construction. Idle = subscribed and waiting (no polling loop).
...
- Secrets: hub-side `.env` (API keys, HF token, git credentials) never enters the public repo; a `.env.example` documents required variables. Workers receive only their git token + CA cert; per-job env passthrough is explicit (`JOB_ENV_*`).
...
| **Workers** (N) | mid (varies by task) | Execute one task per container: checkout task branch, run workloads, push to task branch, report status, write reports + retrospectives | Approve their own gates; push outside their task branch |
...
*Implementation status:* director + auditor are live (`hub/src/agents.ts`). Librarian (secretary role, R6) and reflector (M9) are designed; role-shaping skills live framework-side (`skills/` — migrated in R7).
...
**Exit gates**
1. Worker completes a task: pushes artifacts + evidence to its **task branch** and reports status via API (evidence = committed files: metrics, seeds, configs; reproducibility fields are first-class).
2. **Mechanical gates** read the pushed tree from the bare repo; the **auditor** verifies criteria and that claims are evidenced/reasonable on the same committed state.
3. Pass → task is **audited-complete**; the hub merges the task branch into `main` (§3.2.1). Fail → routes back to the worker for **repair** (SSE instruction; next iterations append on the same branch).
...
- **Retrospective prompt** (authored by the librarian as part of the work handoff).
- **Repair and rebase instructions** (the latter carries the target `main` SHA + the one-time force authorization for the task ref; §3.2.1).
- Future hub-pushed work of any kind.

**Session model:** the session id lives exactly as long as the worker container; each session has its own SSE endpoint (fresh buffer — no last-event-id needed). Idle workers simply wait for events. Instructions target the worker identity and are buffered hub-side while no live session exists (e.g. across the restart window between tasks). Worker→hub remains git push (content) + API calls (status/progress/acks).
...
| HF/git lineage gaps | Lineage index rows record git commit + HF revision together; librarian indexes both |
...
| Worker-restart storms / task loss | Exit-after-task is a single clean exit (not crash loops); leases + requeue already proven; instructions buffered across the restart window |
...
- SSE instruction event schema: envelope (kind, job/branch correlation, payloads incl. rebase target SHA and librarian operational details), ack semantics, offer/accept handshake for work assignment (R4).
...
- Hub-side git access for gates/auditor/librarian: git CLI in the hub container vs libgit2-style library (R5 implementation choice).
- Librarian agent scope (R6): prompt/tool surface for the secretary role; which follow-up request types exist at first release.
