...
- A **pre-receive hook** (policy file generated by the hub) accepts a push only if: the ref matches the job's task branch, the pusher token matches, and the push is a **fast-forward** — pushes to any other ref, including `main`, are **denied server-side**. One carve-out: a **hub-granted, one-time rebase/force authorization** on that task ref (see landing below). Repair iterations and lease-expiry requeues **append** on the same branch — partial dead work stays visible for audit. The hook is thin: it calls a supervisor API that validates the push and **atomically consumes** any one-time authorization (no shared-policy-file races); a consumed-but-failed push re-grants automatically at the next landing attempt. Ref **deletions and tag pushes are denied outright**.
...
- SSE instruction event schema: envelope (kind, job/branch correlation, payloads incl. rebase target SHA and secretary operational details), ack semantics, offer/accept handshake for work assignment; **instruction addressing and buffering semantics** (identity model, at-least-once + idempotent acks, bounded buffers) (R4).
